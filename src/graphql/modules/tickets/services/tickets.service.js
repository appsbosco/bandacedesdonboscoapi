...
function getStrictTicketEmailTemplate(data) {
  ...
  return buildStyledResendTicketEmail(data);
}
...
function formatEmailEventDate(value, locale = "es-CR") {
  if (!value) return "";
...
function buildStyledResendTicketEmail({
  ticket,
  event,
  user,
  buyerName,
  ...
  const recipientName = escapeEmailHtml(
    buyerName ||
  ...
  const recipientEmail = escapeEmailHtml(buyerEmail || user?.email || "");
  const eventName = escapeEmailHtml(event?.name || "Evento BCDB");
  const eventDescription = escapeEmailHtml(
    event?.description || "Tus entradas están listas",
  );
  const eventDate = escapeEmailHtml(formatEmailEventDate(event?.date, locale));
  const ticketId = escapeEmailHtml(ticket?._id?.toString?.() || ticket?.id || "");
  const quantity = Number(ticket?.ticketQuantity || 1);
  const raffleNumbers = Array.isArray(ticket?.raffleNumbers)
    ? ticket.raffleNumbers
    : [];
  const externalNumbers = Array.isArray(ticket?.externalTicketNumbers)
    ? ticket.externalTicketNumbers
    : [];
  const numbers = [...externalNumbers, ...raffleNumbers]
    .map((number) => escapeEmailHtml(number))
    .join(", ");
            ...
            <div style="background:#ffffff;border-radius:3px;overflow:hidden;border:1px solid #dedad4;">
              ...
              <div style="padding:26px 32px 30px;">
                ...
                <table role="presentation" style="width:100%;border-collapse:collapse;font-size:13.5px;margin-bottom:26px;">
                  ${
                    eventDate
                      ? `<tr><td style="padding:8px 0;border-bottom:1px solid #f0ede8;color:#b0a99e;width:100px;vertical-align:top;">Fecha</td><td style="padding:8px 0;border-bottom:1px solid #f0ede8;color:#1a1a18;">${eventDate}</td></tr>`
                      : ""
                  }
                  <tr><td style="padding:8px 0;border-bottom:1px solid #f0ede8;color:#b0a99e;vertical-align:top;">Entradas</td><td style="padding:8px 0;border-bottom:1px solid #f0ede8;color:#1a1a18;font-weight:700;">${quantity}</td></tr>
                  ${
                    numbers
                      ? `<tr><td style="padding:8px 0;border-bottom:1px solid #f0ede8;color:#b0a99e;vertical-align:top;">Números</td><td style="padding:8px 0;border-bottom:1px solid #f0ede8;color:#1a1a18;">${numbers}</td></tr>`
                      : ""
                  }
                  <tr><td style="padding:8px 0;color:#b0a99e;vertical-align:top;">Correo</td><td style="padding:8px 0;color:#1a1a18;">${recipientEmail || "No disponible"}</td></tr>
                  <tr><td style="padding:8px 0;color:#b0a99e;vertical-align:top;">Ticket</td><td style="padding:8px 0;color:#1a1a18;">${ticketId}</td></tr>
                </table>

                <div style="text-align:center;padding:22px 0;">
                  <img alt="Código QR" src="cid:qrCode" width="180" style="display:block;margin:0 auto;border:1px solid #dedad4;" />
              ...
              <div style="padding:18px 32px;border-top:1px solid #eeebe6;background:#faf8f5;text-align:center;">
                <p style="margin:0;font-size:12px;line-height:1.7;color:#9b948a;">
                  Banda CEDES Don Bosco
                </p>
              </div>
            </div>
